...
//        index     = new LinHashMap <> (KeyType.class, Comparable [].class);
    ...
    public Table project (String attributes)
    {
        ...
        for (Comparable [] t : this.tuples) {
            Comparable [] c = new Comparable [attrs.length];
    ...
    public Table union (Table table2)
    {
        ...
        //first adds all of initial table tuples
        for (Comparable [] a : this.tuples) {
            rows.add(a);
        }

        //adds tuples from the second table that aren't in the initial table
        for (Comparable [] b : table2.tuples) {
            for (int i = 0; i < rows.size(); i++) {
                //checks if the tuple from table2 is in the new list of tuples
                if(rows.get(i).equals(b)) {
                    inA = true;
                }
            }
            //adds tuples that aren't in initial table
            if(!inA) {
                rows.add(b); 
            }
            inA = false;
    ...
    public Table minus (Table table2)
    {
        ...
        for (Comparable [] t1 : this.tuples) {
            for (Comparable [] t2 : table2.tuples) {
                KeyType k1 = new KeyType(t1);
                KeyType k2 = new KeyType(t2);
                if (k1.equals(k2))
                    b = true;
            } //for
            if (!b)
    ...
    public Table join (String attributes1, String attributes2, Table table2)
    {
        ...
        int ind1 [] = match(t_attrs);
        int ind2 [] = match(u_attrs);
        
        for(Comparable [] t : this.tuples) {
            for(Comparable [] u : table2.tuples) {
                boolean equal = true;
                Comparable [] tAttr = new Comparable [t_attrs.length];
                Comparable [] uAttr = new Comparable [u_attrs.length];

                //gets tuple from target attribute from this table
                for (int i = 0; i < t_attrs.length; i++) {
                    tAttr[i] = t[ind1[i]];
                } //for

                //gets tuple from target attribute from table2
                ...
                //adds the row of the tuples if they are equal to each other
                if(equal) {
        ...
        //compares each attribute from both tables and then concatinates a "2" on any duplicate in the second table
        String [] table2_Attributes = table2.attribute.clone(); 
        for(int a = 0; a < this.attribute.length; a++) {
            for (int b = 0; b < table2.attribute.length; b++) {
                if(this.attribute[a].equals(table2.attribute[b])) {
                    table2_Attributes[b] = table2.attribute[b] + "2";
                }
            }
        } 
    ...
    * Join this table and table2 by performing an "equi-join".  Tuples from both tables
    * are compared requiring attributes1 to equal attributes2.  Disambiguate attribute
    * names by append "2" to the end of any duplicate attribute name. This is an indexed 
    *
    * @author  Lin Zhao
    *
    * #usage movie.join ("studioNo", "name", studio)
    ...
    * @return  a table with tuples satisfying the equality predicate
    */
    public Table join2 (String attributes1, String attributes2, Table table2) {
    ...
    public Table join (Table table2)
    {
        ...
        for (String s1 : this.attribute) {
            for (String s2 : table2.attribute) {
                if (s1.equals(s2)) {
                    attr.add(s1);
                } //for
            } //for
        } //for
        String [] columns = new String[attr.size()];
        attr.toArray(columns);
        //search for all tuples that are equal for common attributes
        for (Comparable[] t1 : this.tuples) {
            for (Comparable[] t2 : table2.tuples) {
                Comparable[] n1 = this.extract(t1, columns); //new tuple that has only common attr
                Comparable[] n2 = this.extract(t2, columns);
                KeyType k1 = new KeyType(n1);
                KeyType k2 = new KeyType(n2);
                //check equality, if so add to new table
                if (k1.equals(k2)) {
